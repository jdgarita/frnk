...
/**
 * The toolkit's **batteries-included app root** — `FrnkAppShell` plus everything only `:shared` can
 * wire: a fail-fast check that [initializeFrnk] ran, a Settings tab driven by the **live**
 ...
 *
 * ```kotlin
 * // Application.onCreate (Android) — iOS calls the common initializeFrnk(...) before the VC.
 ...
 * ```
 *
 * Under `MonetizationChoice.None` (no [EntitlementManager] in the graph) the Settings tab renders
 * with `isPro = false` and monetization rows degrade to no-ops; the paywall entry is not mounted.
        ...
        remember {
            checkNotNull(KoinPlatformTools.defaultContext().getOrNull()) {
                "Koin is not started. Call initializeFrnk(...) before composing FrnkAppScaffold — " +
                    "from Application.onCreate(context = this) on Android, or before creating the " +
                    "ComposeUIViewController on iOS."
            }
        }
    // Monetization is an optional axis (MonetizationChoice.None installs no EntitlementManager);
    // resolve leniently and degrade the Settings tab + paywall when absent.
