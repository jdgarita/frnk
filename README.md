...
| `ui-app` | The batteries-included app root: `FrnkAppScaffold` (over `ui-bottom-nav`'s `FrnkTabbedNavScaffold` — live entitlement-aware Settings, auto-mounted paywall, Koin fail-fast) + `frnkUiModules()` (the SDK-free scaffold VM modules). No `*-impl` compile deps. |
...
| `core-nav` | The Compose-free **type-safe Navigation3 contract**: `ToolkitRoute`, the `NavBackStack<NavKey>` mutation helpers, `frnkNavConfiguration(...)`, the `FrnkPendingRouteRequest` deep-link signal, and the `FrnkFullScreenRoute` marker. No `compose.runtime`. |
...
| `ui-scaffolds` | The **page templates** + the Compose binding layer for the MVI/nav engines: scaffolds (`OnboardingScreen`, `SettingsScreen`, `HomeScreen`, `FrnkScreenScaffold`), `FrnkMviScreen` + `EffectCollector`, and `FrnkNavDisplay` + `rememberFrnkNavBackStack` + `rememberFrnkTabbedBackStacks` + the slide animations. |
| `ui-bottom-nav` | **Platform-adaptive bottom navigation** — `FrnkBottomFloatingBar`, an `expect`/`actual` composable: a Material3 *Expressive* `HorizontalFloatingToolbar` (floating pill) on Android and a native glassy `UITabBar` (iOS 26+) / Material3 bar (older) on iOS (via [adaptive-nav-bar](https://github.com/narendraanjana09/adaptive-navigation-bar)), both themed from `FrnkTheme` tokens. The bar is a fixed three-tab shape — `Home · feature · Settings` — whose center "feature" tab is the host's only configurable slot (`FrnkFeatureItem`, a real navigable tab), plus `FrnkTabbedNavScaffold` (the **one-call tabbed app** — theme wrap + nav config + the fixed tabs + per-tab back stacks + built-in Home/Settings/Onboarding destinations + deep-links, wrapping the nav3 multiple-back-stack render core that wires the display + bar + tab switching + back convention + bar inset). Icons are `ImageVector` (Android) + SF-Symbol string (iOS). **The toolkit's sole Material3 dependency**, deliberately isolated here so `ui-theme`/`ui-components`/`ui-scaffolds` stay `compose-unstyled`-only. Android never touches `DrawableResource`, so there's **no host-side asset step**. |
