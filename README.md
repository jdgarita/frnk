...
| `core-nav` | The Compose-free **type-safe Navigation3 contract**: `FrnkRoute`, the `NavBackStack<NavKey>` mutation helpers, `frnkNavConfiguration(...)`, the `FrnkPendingRouteRequest` deep-link signal, and the `FrnkFullScreenRoute` marker. No `compose.runtime`. |
