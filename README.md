...
| `core-nav` | The Compose-free **type-safe Navigation3 contract**: `ToolkitRoute`, the `NavBackStack<NavKey>` mutation helpers, `frnkNavConfiguration(...)`, the `FrnkPendingRouteRequest` deep-link signal, `FrnkPrimaryActionRegistry`, and the `FrnkFullScreenRoute` marker. No `compose.runtime`. |
...
| `ui-bottom-nav` | **Platform-adaptive bottom navigation** — `FrnkAdaptiveNavBarBottomBar` rendering a native glassy `UITabBar` (iOS 26+) / Material3 bar (older) on iOS and a Material3 `NavigationBar` on Android (via [adaptive-nav-bar](https://github.com/narendraanjana09/adaptive-navigation-bar), themed from `FrnkTheme` tokens, with a built-in primary-action FAB), plus `FrnkTabbedNavScaffold` (the nav3 multiple-back-stack tabbed scaffold — one call wires the display + bar + tab switching + back convention + bar inset). **The toolkit's sole Material3 dependency**, deliberately isolated here so `ui-theme`/`ui-components`/`ui-scaffolds` stay `compose-unstyled`-only. The bar's `DrawableResource` icons don't package on Android from a KMP library module under this toolchain — Android hosts ship the nav drawables as raw assets (see `docs/HOST_INTEGRATION.md`). |
