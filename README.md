...
| `ui-app` | The batteries-included app root: `FrnkAppScaffold` (over `ui-bottom-nav`'s `FrnkAppShell` — live entitlement-aware Settings, auto-mounted paywall, Koin fail-fast) + `frnkUiModules()` (the SDK-free scaffold VM modules). No `*-impl` compile deps. |
