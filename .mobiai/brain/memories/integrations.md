...
- The toolkit never calls `FirebaseApp.configure()` — the host app does (Android `google-services` plugin; iOS `FirebaseApp.configure()` in Swift + `GoogleService-Info.plist`).
