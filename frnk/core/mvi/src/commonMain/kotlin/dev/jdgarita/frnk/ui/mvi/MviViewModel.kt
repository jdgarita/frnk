...
/**
 * Model-first MVI base — the toolkit's single ViewModel base for every feature screen.
 *
...
abstract class MviViewModel<A : Arguments, M : ModelState, S : UiState, I : UiIntent, E : UiEffect>(
    ...
    fun onIntent(intent: CommonUiIntent) =
        launchAttached {
            when (intent) {
                CommonUiIntent.OnBackPressed -> emit(effect = CommonUiEffect.DidPressBack())
            }
        }
