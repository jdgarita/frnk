...
/**
 * Verifies the [ModelMviViewModel] engine via a minimal in-test subclass: the initial UiState is the
 * factory's model mapped through [ModelMviViewModel.mapToUiState]; mutating the model via
 * [ModelMviViewModel.updateModel] re-derives the UiState automatically; and an intent emits a one-shot
 * effect. The reusable template for testing real model-first reducers.
 */
class ModelMviViewModelTest {
...
private class SampleViewModel :
    ModelMviViewModel<SampleArguments, SampleModel, SampleUiState, SampleIntent, SampleEffect>(SampleModelFactory) {
