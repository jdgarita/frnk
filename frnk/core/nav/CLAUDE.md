...
- Stick to the Compose-free nav3 runtime — no `navigation3-ui`, no `compose.runtime`. The Compose nav engine (`FrnkNavDisplay`, tabbed stacks, animations) belongs in `:shared-ui-atoms`.
...
- Feature modules emitting route effects should depend on **this** module, not on `:shared-ui-atoms`, when they don't need composables.
