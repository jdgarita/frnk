...
- `ui/nav/FrnkRootRoute.kt` — a **parallel root-level** catalogue (`Onboarding`, `Tab`, `Paywall`, `Custom(id)`) for the lower-level `FrnkApp` root path (`:ui-app`): a two-level shape where modal/full-screen flows (`Onboarding`, `Paywall`) and the tabbed shell (`Tab`, which hosts an inner `FrnkRoute` back stack) live at the app root, while `FrnkRoute` keys the nested tab stack inside `Tab`. `Onboarding`/`Paywall` carry `FrnkFullScreenRoute`. **Coexists with** `FrnkRoute` — the batteries-included `FrnkAppScaffold`/`FrnkTabbedNavScaffold` path keeps using `FrnkRoute` directly; `FrnkRootRoute` is for hosts wiring the graph themselves over `FrnkApp`.
