...
- `initializeFrnk(modules: List<Module>, validate: Boolean = false, validator: (Koin) -> Unit = {}, extraConfig: KoinApplication.() -> Unit = {}): KoinApplication` — one-shot `startKoin`. Hosts pass an **explicit module list** (`frnkUiModules() + databaseModule + prefsModule + postHogAnalyticsModule(config) + sentryCrashReportingModule(config) + …`); there is no backend/observability/monetization switch. See `docs/HOST_INTEGRATION.md` §4 for the canonical snippet. The additive `validate`/`validator` params (Tier 2.2, default off/no-op) run `validator(koin)` right after `startKoin` for fail-fast bootstrap checks — but **the rule set lives in `:ui-app`** (`Koin::validateFrnkBootstrap`), not here: this module names no capability type and just runs whatever check it is handed, preserving the OQ-7 "no capability knowledge" rule.
- **androidMain** `initializeFrnk(context, modules, validate, validator, extraConfig)` — also sets `DatabaseContext.application` and registers `androidContext(...)`; forwards `validate`/`validator` to the common overload. iOS hosts call the common overload.
- **androidMain** `DatabaseContext` — the process-wide Android `Context` seam the data layer reads (`:data-db-api`'s Room builder, `:data-db-impl`'s database locations, `:data-prefs-impl`'s SharedPreferences-backed `KeyValueStore`). Re-homed here at Stage 4 (it lived in the old `shared-database-impl`) so the two split impls share it without depending on each other; they now depend on this module, not the reverse. Hosts that bypass `initializeFrnk` must set it from `Application.onCreate` (the demo does).
