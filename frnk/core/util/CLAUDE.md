...
- `AppResult.kt` — the **toolkit-wide result envelope**: `sealed interface AppResult<out D, out E : AppError>` (`Success(data)` / `Failure(error)`), the `AppError` interface (`val message: String`), the `CommonError` enum (`Network`, `Unauthorized`, `NotFound`, `Unknown`), and the `fold(onSuccess, onFailure)` extension. Every `*-api` interface returns this and never throws. It lives here (the neutral root) rather than in `:shared:backend:api` so backend, persistence, monetization apis (and the demo's `NoteStore`) can all return it without a sibling `*-api`→`*-api` dependency. Tested by `AppResultTest` in `commonTest`.
