 ...
 *
 * Single source for the inset read by the adaptive bar's `reservedHeight` (`FrnkBottomFloatingBar` in
 * `:ui-bottom-nav`, which `api`-depends on this module), so the `WindowInsets.navigationBars` lookup
 * isn't duplicated.
 */
@Composable
