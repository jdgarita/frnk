...
- `ui/scaffolds/` — higher-than-atom, lower-than-feature page templates with a fixed shape + a configurable state class. Each scaffold ships **two** entry points: a stateless `*Content` composable (previews + advanced hosts), and a VM-backed convenience wrapper resolving an `Mvi` ViewModel via Koin (`koinViewModel { parametersOf(initialState) }`). A per-scaffold Koin module (`xxxScaffoldModule`) registers the VM; downstream modules `includes(...)` it. Default copy comes from `FrnkStrings` (`:ui-theme`); hosts override per-token via `FrnkThemeConfig.stringOverrides`.
  - `OnboardingScreen.kt` / `OnboardingScreenState.kt` / `OnboardingViewModel.kt` / `OnboardingScaffoldModule.kt` — close-X + `HorizontalPager` (configurable `pagerHeight`) + animated pip indicator + Back/Next → "Get Started" on the last page. Pure UI-state machine.
  ...
  - `BottomNavScaffold.kt` / `BottomNavScaffoldState.kt` / `BottomNavViewModel.kt` / `BottomNavScaffoldModule.kt` / `BottomNavDefaults.kt` — the **index-based** (not nav3) bottom-nav scaffold: owns the selected tab + overlays the floating `FrnkBottomNavBar` over the full-height destination. `rememberBottomNavScaffoldState(middleTab, …)` builds the 3-tab Home + middle + Settings state.
