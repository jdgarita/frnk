...
- `ui/theme/` — the theme engine.
  ...
  - `FrnkStringSource.kt` / `FrnkIconSource.kt` (+ `ext/` `resolve()` extensions) — **token-in-state refs**: `FrnkStringSource` (`Token`/`Raw`/`Composite`) and `FrnkIconSource` (`Token`/`Vector`). View states hold these (Compose-free authoring); the `@Composable resolve()` extensions map them to `String`/`ImageVector` at the leaf atom (`FrnkText`/`FrnkIcon`), so a host's `stringOverrides`/`iconOverrides`/locale re-resolve automatically. `Composite` is **titles-only** (its `resolve()` allocates per call — not for list/row content).
