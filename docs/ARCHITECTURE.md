...
core/   (no upward deps; util is the root everything depends on)
  ...
  nav   ── Navigation3 contract, FrnkRoute, registries (no Compose)
...
ui/   (floor → ceiling; haptics ← theme ← components ← scaffolds ← bottom-nav ← app)
  ...
  bottom-nav  ── adaptive bottom nav + FrnkTabbedNavScaffold  (the SOLE Material3 module)
...
holds, and the demo's shared composable can use the batteries-included
`FrnkAppScaffold` on both platforms.) This keeps
...
- **`:core-nav`** (`ui/nav/`) holds the Compose-free contract (the nav3 *runtime* is pure Kotlin/MP): `FrnkRoute` — a `@Serializable sealed interface FrnkRoute : NavKey` of default **tab-level** routes (`Home`/`Onboarding`/`Settings`/`Paywall`/`Custom`) — the back-stack mutation helpers (`NavBackStack<NavKey>.navigateTo(route, popScreen?, singleTop=true)` / `back()` / `clearAndNavigateTo(route)`), the `SavedStateConfiguration` builders (`val frnkRootNavConfig` for the **root** stack registering `FrnkRootRoute`, and `frnkNestedNavConfig(hostRoutes)` for a **nested/tab** stack registering `FrnkRoute.Home`/`Custom`/`Settings` plus the host's own polymorphic routes — together they replace the old single `frnkNavConfiguration`, which is gone), the `FrnkPendingRouteRequest` deep-link signal, and the `FrnkFullScreenRoute` marker (a route implementing it hides the tabbed scaffold's bottom bar — `FrnkTabbedNavScaffold`'s default `hideBarFor`). It also holds `FrnkRootRoute` — a **parallel root-level** catalogue (`Onboarding`/`Tab`/`Paywall`/`Custom`) for the lower-level `FrnkApp` two-level root path (`:ui-app`), coexisting with `FrnkRoute`'s batteries path. Routes are `@Serializable` (needs `kotlinx-serialization-core` — **not** `-json`; nav3 encodes routes via savedstate's `SavedStateEncoder`) and depend on `androidx-navigation3-runtime` (`NavKey`/`NavBackStack`). No Compose here, so feature ViewModels and effect handlers compile without `compose.runtime`.
...
- **`ui-bottom-nav`** (`ui/bottomnav/`) holds the one-call **tabbed app**: `FrnkTabbedNavScaffold(appVersion, feature, …) { homeContent }` is the single public composable over everything above — theme wrap, nav config, the fixed `Home · feature · Settings` tabs (center tab supplied as a `feature: FrnkFeatureItem`) with per-tab back stacks, built-in Home (`HomeScreen` content slot) / Settings (default catalogue + `extraSections`) / optional Onboarding destinations, and deep-links; each host extension point is handed a `FrnkAppScope` (`navigateTo`/`back`), and the host registers the feature tab's route in `entries`. Its nav3 multiple-back-stack **render core** (the `FrnkNavDisplay` + persistent platform-adaptive bottom bar + tab switching / re-tap-to-root + `FrnkTabbedBackHandler` + full-screen bar hiding, `hideBarFor` defaulting to `{ it is FrnkFullScreenRoute }` + the bottom-inset via `LocalFrnkBottomBarInset`) is a **private helper** in the same file. (This is the merge of the former `FrnkAppShell` + the lower-level `FrnkTabbedNavScaffold` into one component.) The bar (`FrnkBottomFloatingBar`) is an `expect`/`actual` composable — a Material3 Expressive `HorizontalFloatingToolbar` on Android, a native glassy `UITabBar` (over [adaptive-nav-bar](https://github.com/narendraanjana09/adaptive-navigation-bar)) on iOS; for multiple-back-stack nav3 without Material3 — or a tab shape other than the fixed three — hand-wire the primitives (`rememberFrnkTabbedBackStacks` + `FrnkNavDisplay` + `FrnkTabbedBackHandler` + a custom bar). The bar is a **fixed three-tab** shape — `Home · feature · Settings` — whose center "feature" tab is the host's only configurable slot (`FrnkFeatureItem`, a real navigable tab); `rememberFrnkBottomNavState(homeRoot, settingsRoot, feature)` builds the fixed `FrnkBottomNavState` with the toolkit-owned Home/Settings bookends. Bar icons are `ImageVector` (Android) + SF-Symbol string (iOS); Android never touches `DrawableResource`, so there is **no host-side asset step**.
- `ui-bottom-nav` (`ui/bottomnav/`) additionally holds **`FrnkAppScope`** (the navigation handle handed to every extension point) and **`FrnkFirstLaunchOnboardingEffect`** (the first-launch gate helper). **`:ui-app`** layers **`FrnkAppScaffold`** on top (Koin fail-fast assertion via `:core-di`'s `requireFrnkKoin()`, live `EntitlementManager`-driven Settings, `rememberFrnkSettingsHandler`, auto-mounted `FrnkRoute.Paywall`, and **first-launch onboarding** auto-presented once via a `KeyValueStore`-backed gate when `onboardingPages` are supplied) — the host-facing "whole app in one call" entry point (`docs/HOST_INTEGRATION.md` §8); Android hosts bootstrap with the androidMain `initializeFrnk(context, modules)` overload (`:core-di`) first. `:ui-app` also ships **`FrnkApp(onSavedStateConfiguration, onNavigationModule)`** — a **lower-level** root that coexists below `FrnkAppScaffold`: it owns only theme/appearance/system-bars + a single root `NavDisplay` over `FrnkRootRoute` (seeded at `Onboarding`) and hands the graph to the host via a Koin `navigation<Route> { … }` module, for hosts wanting full control over the nav graph instead of the declarative `FrnkAppConfig`. Full-window destinations (onboarding, paywall) render through **`:ui-scaffolds`' `FrnkFullScreenScaffold`** — an immersive template with an always-on ✕ that reserves safe-area insets + the close-button band.

**The toolkit ships the nav display + scaffold machinery; the host owns the back-stack instance** (it creates it via `rememberFrnkNavBackStack` / `rememberFrnkTabbedBackStacks` and passes it in). Navigation is driven by the MVI effect channel: a ViewModel emits a navigation `UiEffect`, and a single `EffectCollector` above the display mutates the host-owned back stack via `navigateTo`/`back`/`clearAndNavigateTo` (collect it in exactly one place — the effect channel is single-consumer). On Android `NavDisplay` consumes the system back button / predictive-back gesture to pop the back stack automatically (the host activity sets `android:enableOnBackInvokedCallback="true"`); on iOS the back-gesture support depends on the Compose Multiplatform runtime, so screens should also carry an on-screen back affordance. `:demo-shared`'s `FrnkDemoApp` is the reference integration — the single shared composable both `demo-android`'s `MainActivity` and `iosDemoApp`'s `MainViewController` call, built over **`FrnkAppScaffold`** (`:ui-app`): it supplies the Home/Settings/Onboarding destinations and three bottom-nav tabs (each keeping its own back stack); the demo registers only its middle "Components" tab and the pushed `ComponentDetail(name)` — the `Paywall` destination and first-launch onboarding are auto-mounted by `FrnkAppScaffold`.
